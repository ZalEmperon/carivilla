        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-dark admin-sidebar collapse">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="{{ route('admin.villas.create') }}">
            ...
            <form action="{{ route('admin.villas.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="jumlah_kamar" name="jumlah_kamar" value="{{ old('jumlah_kamar') }}" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="jumlah_kamar_mandi" class="form-label">Jumlah Kamar Mandi</label>
                        <input type="text" class="form-control" id="jumlah_kamar_mandi" name="jumlah_kamar_mandi" value="{{ old('jumlah_kamar_mandi') }}" required>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="luas_tanah" class="form-label">Luas Tanah (contoh: 500m²)</label>
                        <input type="text" class="form-control" id="luas_tanah" name="luas_tanah" value="{{ old('luas_tanah') }}" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="luas_bangunan" class="form-label">Luas Bangunan (contoh: 200m²)</label>
                        <input type="text" class="form-control" id="luas_bangunan" name="luas_bangunan" value="{{ old('luas_bangunan') }}" required>
                    </div>
                </div>
                <div class="mb-3">
                    <label for="fasilitas" class="form-label">Fasilitas (pisahkan dengan koma atau enter untuk setiap fasilitas)</label>
                    <textarea class="form-control" id="fasilitas" name="fasilitas[]" rows="3" placeholder="Contoh: Kolam Renang, WiFi, Dapur Lengkap">{{ old('fasilitas') ? implode(', ', old('fasilitas')) : '' }}</textarea>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control" id="harga_weekday" name="harga_weekday" value="{{ old('harga_weekday') }}" step="0.01" min="0" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="harga_weekend" class="form-label">Harga Weekend (Rp)</label>
                        <input type="number" class="form-control" id="harga_weekend" name="harga_weekend" value="{{ old('harga_weekend') }}" step="0.01" min="0" required>
                    </div>
                </div>
                <div class="mb-3">
                    <label for="foto_slider" class="form-label">Foto Villa (bisa pilih banyak)</label>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nomor_wa" name="nomor_wa" value="{{ old('nomor_wa') }}" placeholder="Contoh: 6281234567890">
                </div>

                <button type="submit" class="btn primary-button">Simpan Villa</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        fasilitasInput.addEventListener('input', function() {
            ...
            // Update the hidden input for facilities array if needed, or adjust backend validation
            // For now, Laravel's validation 'fasilitas.*' will handle this as an array from the form.
            // This is just for user experience feedback.
        });
    });
</script>
@endpush